...
if (riv) then
   ...
   DO nz=1,nsigma-1
  if (i<=11) then
   aux=ctf(nz,riv_node(i))+dt*ctfold(nz,riv_node(i))*Qr_node_sig(nz,i)&
 ...
 ! if (abs(aux)< 1.e-12) aux=abs(aux)
  ! if (aux<0.0_WP) then
  ! write(*,*) 'Negative concentration, time resolution is insufficient, stop'&
  ! , aux, riv_node(i), ctf(nz,riv_node(i)), nz, Qr_node_sig(nz,i)
  ! stop
  ! else
    ctf(nz,riv_node(i))= aux
  ! endif
   enddo
   enddo
   endif
