...
CONTAINS
   SUBROUTINE define_varlist
      ...
      nvar=nvar+1
      varlist(nvar)%name =           'h_var' ! name
      varlist(nvar)%modelname =      'h_var' ! name of variable in model
      varlist(nvar)%standard_name =  'h_var'! full variable name in the NetCDF file
      varlist(nvar)%units =          'm'! variable units in the NetCDF file
      varlist(nvar)%onnode =         1 ! if on nod =1 , if on elements =0, other =-1
      varlist(nvar)%timedim =        1 ! time dimension 0 (not in time), 1 (in time)
      varlist(nvar)%vertdim =        0 ! vertical dimension 0 , 1 (verticaly resolved) sigma, 2 sigmam1
      ...
      nvar=nvar+1
      varlist(nvar)%name =           'con' ! name
      varlist(nvar)%modelname =      'con' ! name of variable in model
      varlist(nvar)%standard_name =  'con'! full variable name in the NetCDF file
      varlist(nvar)%units =          'mg/l'! variable units in the NetCDF file
      varlist(nvar)%onnode =         1 ! if on nod =1 , if on elements =0, other =-1
      varlist(nvar)%timedim =        1 ! time dimension 0 (not in time), 1 (in time)
      varlist(nvar)%vertdim =        0 ! vertical dimension 0 , 1 (verticaly resolved) sigma, 2 sigmam1
      varlist(nvar)%type_task =      1 ! T,S,... =3, U_n,V_n,... =2, eta_n,... =1
...
!======================   Put your 1D variables here: ========================================
   SUBROUTINE getvar1d(i)
      ...
      select case (varbas1d(i)%varid)
         ...
         case(27)
           varbas1d(i)%snapshot =  h_var
         case(28)
          varbas1d(i)%snapshot  =  con
         case(29)
...
!         READ( unit=filelist_unit, FMT=*,iostat=iost)
   ...
   !==================== define indexes of nodes and elements where to write
      do i = 1, numfiles
         ...
         elseif ( filelist(i)%ftype==3 ) then
         ! station
            call ne_node(filelist(i)%posx_ll, filelist(i)%posy_ll, filelist(i)%nods(1))
