...
!!   W ---> wind speed
!!****************************************
!aa   d1 = 0.5_WP*(sigma(nsigma-1) - sigma(nsigma))
!aa   u_din_sur = 0.05_WP*(windx(node)**2 + windy(node)**2)
!aa   snu_m = 0.5_WP*(snu(nsigma,node) + snu(nsigma-1,node))
!aa   yy1(2) = (0.4d-3*u_din_sur**3*dmean*d1)/(snu_m*0.76_WP)
...
!++++++++++++++++++++++++++++++++++++++++
       !a = max(depth(node), Dmin)
          zt = sigma(nz)*dmean
!aa          Zh = zt + z0b_min
!aa          Zd = abs(dmean - zt) + z0s_min
          Zh = zt + 5.0d-5
          Zd = abs(dmean - zt) + z0b_min
...
!aa	     if (snu(nz,node) >= 1.0_WP) snu(nz,node) = 1.0_WP
!aa	     Kv(nz,node) = Pr_num*(snu(nz,node))
	 Kv(nz,node) = Pr_num*(snu(nz,node)) + snul
        end do
...
!+++++++++++++++++++++++++++++++++++++++
    ...
    b2 = -0.5_WP*g*Kv(nz,node)*ap1/(density_0*dmean)
      ...
      Ri(nz) = -g*ap1/(density_0*dmean)/M_pr
...
USE o_ARRAYS, only: taux_node, tauy_node,taux, tauy, Unode, Vnode, Kv,Kv2, Av, Av_node, teps, tke, L, eta_n, &
TF, SF, CF, zbar, Jc , z0b_gotm_el

!--Variables from GOTM Modules-------------------------------

USE turbulence, only: do_turbulence
USE turbulence, only: tke1d => tke, eps1d => eps, l1d => L, num, nuh, nus !Shear and buoyancy productions can be included  P1d => P, B1d => B, if it is necessary
...
!   call compute_el2nodes_2D_2fields(taux,tauy,taux_node,tauy_node)
 ...
 !Set BC's as GOTM Does
  ...
  z0b_gotm=z0b_gotm_nodes(node)
      ...
      nus(0:nsigma-1)   = Kv2(nsigma:1:-1,node)     !Vertical Kinematic Viscosity  [m^2/s] 
      ...
      !Update 3D Fields of tke,eps,Av(KM),Kv(KH) and tlen, you can also include shear and buoyancy productions P,B, if it is necessary
       ...
       Av_node(1:nsigma, node) = min(num(nsigma-1:0:-1),107.0_WP)+ snul !eddy viscosity
       Kv(1:nsigma, node) =   min(nuh(nsigma-1:0:-1),107.0_WP) + snul    !eddy diffusivity
       Kv2(1:nsigma, node) =   min(nus(nsigma-1:0:-1),107.0_WP) + snul !eddy diffusivity salinity
