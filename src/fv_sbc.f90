...
!      write(*,*) att_string
   ...
   SUBROUTINE nc_sbc_ini_fillnames(yyear)
      ...
      write(sbc_flfi(i_xwind)%file_name,*) trim(nm_xwind_file)
      write(sbc_flfi(i_ywind)%file_name,*) trim(nm_ywind_file)

      write(sbc_flfi(i_humi)%file_name, *) trim(nm_humi_file)
      write(sbc_flfi(i_qsr)%file_name, *) trim(nm_qsr_file)
      write(sbc_flfi(i_qlw)%file_name, *) trim(nm_qlw_file)
      write(sbc_flfi(i_tair)%file_name, *) trim(nm_tair_file)
      write(sbc_flfi(i_prec)%file_name, *) trim(nm_prec_file)
      write(sbc_flfi(i_mslp)%file_name, *) trim(nm_mslp_file)
      if (nm_calc_flux==1) then
         write(sbc_flfi(i_cloud)%file_name, *) trim(nm_cloud_file)
      end if
...
!!$OMP PARALLEL
!!$OMP DO
!!      write(*,*) 'MAXWIND: ', maxval(atmdata(i_xwind,:)), maxval(atmdata(i_ywind,:))
!!      write(*,*) 'MAXWIND: ', minval(atmdata(i_xwind,:)), minval(atmdata(i_ywind,:))
      do i = 1,nod2D
...
!ik         taux_node(i) = rtmp * wdx  ! new taux (stress along x)
!ik         tauy_node(i) = rtmp * wdy  ! new tauy (stress along y)
         taux_node(i) = rhoa * wndm * (0.6 + 0.07 * wdx ) /1000 * wdx  ! new taux (stress along x)
         tauy_node(i) = rhoa * wndm * (0.6 + 0.07 * wdy ) /1000 * wdy  ! new taux (stress along x)

!         taux(i) = rhoa * wdx * wdx * (0.6 + 0.07 * wdx ) /1000
!         tauy(i) = 0.0
!!!===========================================  changing of taux and tauy here ====================

     !aa67    zevap = MAX( 0.e0, rhoa    *Ce*( q_sat - q_zu ) * wndm )   ! Evaporation
     !aa67    zqsb  = rhoa*cpa*Ch*( zst - t_zu ) * wndm     ! Sensible Heat
...
!                data1(elem2D),data2(elem2D), &
      ...
      now_date = rdate
!write(*,*) 'rdate',  rdate
!write(*,*) 'nc_Ntime, nc_time, now_date', nc_Ntime, nc_time, now_date
...
!write(*,*) 't_indx=', t_indx, 'nc_Ntime=', nc_Ntime
      ...
      do fld_idx = 1, itot
         if (fld_idx==1 .or. fld_idx==2 .or. fld_idx==8) then
         !open file sbc_flfi
...
!
          if (fld_idx==1) then
           sbcdata1=sbcdata1*0.000825062443939265 + 2.13191168752803
           sbcdata2=sbcdata2*0.000825062443939265 + 2.13191168752803
          endif
          if (fld_idx==2) then
           sbcdata1=sbcdata1*0.000816561640834222 + 1.42573155072255
           sbcdata2=sbcdata2*0.000816561640834222 + 1.42573155072255
          endif
          if (fld_idx==8) then
           sbcdata1=sbcdata1*0.13188679749134 + 100097.090306601
           sbcdata2=sbcdata2*0.13188679749134 + 100097.090306601
          endif
