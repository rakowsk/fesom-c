...
real(kind=WP)                :: mnv,mxv !TEMPORARY AUXILIARY VARIABLES
...
subroutine par_init    ! initializes MPI
  ...
  MPI_COMM_FESOM_C=MPI_COMM_WORLD
#else
  call MPI_Comm_Size(MPI_COMM_FESOM_C,npes, i)
  call MPI_Comm_Rank(MPI_COMM_FESOM_C,mype, i)
#endif  
...
subroutine par_ex(abort)       ! finalizes MPI
  ...
  if (mype==0) print *,'MPI has been finalized'
...
!================================================
! MPI REQUEST BUFFERS (3 for nodes elems and elems_full)
!================================================
...
!!$      deallocate(displace, blocklen)
      ...
      allocate(r_mpitype_elem3D(com_elem2D%rPEnum, nsigma-1:nsigma,4))     ! 3D, small halo 
      allocate(s_mpitype_elem3D(com_elem2D%sPEnum, nsigma-1:nsigma,4))

      allocate(r_mpitype_elem3D_full(com_elem2D_full%rPEnum, nsigma-1:nsigma,4))     ! 3D, wide halo
      allocate(s_mpitype_elem3D_full(com_elem2D_full%sPEnum, nsigma-1:nsigma,4))
  ...
  blocklen_tmp(1:nb) = blocklen(1:nb)*n_val !SH replaced blocklen by blocklen_tmp
  displace_tmp(1:nb) = displace(1:nb)*n_val !SH replaced displace by displace_tmp
  ...
  displace_tmp(1:nb) = displace(1:nb)*n_val !SH replaced displace by displace_tmp
   ...
   ! Build MPI Data types for halo exchange: Nodes
      ...
      allocate(r_mpitype_nod3D(com_nod2D%rPEnum,nsigma-1:nsigma,3))  ! 3D with nsigma-1 or nsigma layers, 1-3 values 
      allocate(s_mpitype_nod3D(com_nod2D%sPEnum,nsigma-1:nsigma,3))
  ...
  if (mype==0) then
     ...
     if (npes > 1) then
        ...
        do n=1, npes-1
           ...
           call MPI_RECV(remList_nod2D(nstart), n2D, MPI_INTEGER, n, 2, MPI_COMM_FESOM_C, &
                                                           MPI_STATUS_IGNORE, MPIerr ) 
           estart = remPtr_elem2D(n)
           e2D    = remPtr_elem2D(n+1) - remPtr_elem2D(n)
           call MPI_RECV(remList_elem2D(estart),e2D, MPI_INTEGER, n, 3, MPI_COMM_FESOM_C, &
                                                           MPI_STATUS_IGNORE, MPIerr ) 
     ...
     call MPI_SEND(myList_nod2D,  myDim_nod2D,  MPI_INTEGER, 0, 2, MPI_COMM_FESOM_C, MPIerr )
     call MPI_SEND(myList_elem2D, myDim_elem2D, MPI_INTEGER, 0, 3, MPI_COMM_FESOM_C, MPIerr )
